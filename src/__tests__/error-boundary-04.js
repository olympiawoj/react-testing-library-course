...
test('calls reportError and renders that there was a problem', () => {
  ...
  const {rerender, getByText, getByRole, queryByRole, queryByText} = render(
    <Bomb />,
    {wrapper: ErrorBoundary}, // wrapper option = this will take the UI we provide in 1st arg and wraps it in the ErrorBoundary component.
  ) // render with Bomb component
  rerender(<Bomb shouldThrow={true} />) // when we do this, we makes a call to reportError, throw error, componentDidCatch should run, re render with error info
  // ANY time we call rerender, this Bomb will be wrapped in the ErrorBoundary
